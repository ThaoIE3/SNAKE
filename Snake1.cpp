...
void vemoto(int dong,int cot,int kitu,int mau=12)
{
...
void HienThi(Snake snake, HoaQua hq)
{
    ...
    for(int i=0;i<consoleHeight+1;i++)
        vemoto(i,consoleWidth,180);
    for(int i=0;i<consoleWidth+1;i++)
        vemoto(consoleHeight-1,i,193);
    for(int i=0;i<consoleWidth+1;i++)
        vemoto(0,i,194);
    for(int i=0;i<consoleHeight;i++)
        vemoto(i,0,195);
    vemoto(hq.td.y,hq.td.x,'+');
    vemoto(snake.dot[0].y,snake.dot[0].x,'@');
...
int XuLy(Snake &snake, HoaQua &hq,int &ThoiGianSleep)
{
    ...
    if(snake.dot[0].x==hq.td.x&&snake.dot[0].y==hq.td.y)
    {
        ...
        if(snake.tt==UP)
            ...
            snake.dot[0].x--;
        hq.td.x=1+rand()%consoleWidth;
        hq.td.y=1+rand()%(consoleHeight-1);
        if(ThoiGianSleep>30)
            ThoiGianSleep-=20;
    }
}
