...
int XuLy(Snake &snake, HoaQua &hq,int &ThoiGianSleep)
{
    ...
    if(snake.dot[0].x==hq.td.x&&snake.dot[0].y==hq.td.y||
       snake.dot[0].x==hq.td.x&&snake.dot[0].y==hq.td.y+1||
       snake.dot[0].x==hq.td.x+1&&snake.dot[0].y==hq.td.y||
       snake.dot[0].x==hq.td.x+1&&snake.dot[0].y==hq.td.y+1)
    {
        ...
        if(snake.tt==UP)
            ...
            snake.dot[0].x--;
        hq.td.x=1+rand()%(consoleWidth-2);
        hq.td.y=1+rand()%(consoleHeight-3);
        if(ThoiGianSleep>30)
            ThoiGianSleep-=20;
    }
}
